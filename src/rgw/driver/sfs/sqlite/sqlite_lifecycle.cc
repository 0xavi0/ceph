...
std::optional<DBOPLCEntry> SQLiteLifecycle::get_entry(const std::string & oid,
...
std::optional<DBOPLCEntry> SQLiteLifecycle::get_next_entry(const std::string & oid,
                                            const std::string & marker) const {
  auto storage = conn->get_storage();
  auto db_entries = storage.get_all<DBOPLCEntry>(where(
                                                is_equal(&DBOPLCEntry::lc_index, oid) and
                                                greater_than(&DBOPLCEntry::bucket_name, marker)),
                                                order_by(&DBOPLCEntry::bucket_name).asc(),
                                                limit(1));
  std::optional<DBOPLCEntry> ret_value;
...
void SQLiteLifecycle::remove_entry(const std::string & oid,
                                   const std::string & marker) const {
  auto storage = conn->get_storage();
  storage.remove<DBOPLCEntry>(oid, marker);
}

std::vector<DBOPLCEntry> SQLiteLifecycle::list_entries(const std::string & oid,
                                                  const std::string & marker,
                                                  uint32_t max_entries) const {
  auto storage = conn->get_storage();
  return storage.get_all<DBOPLCEntry>(where(
                                  is_equal(&DBOPLCEntry::lc_index, oid) and
                                  greater_than(&DBOPLCEntry::bucket_name, marker)),
                                  order_by(&DBOPLCEntry::bucket_name).asc(),
                                  limit(max_entries));
