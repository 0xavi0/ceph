...
inline auto _make_storage(const std::string &path) {
  return sqlite_orm::make_storage(path,
    ...
    sqlite_orm::make_table(std::string(LC_HEAD_TABLE),
          sqlite_orm::make_column("lc_index", &DBOPLCHead::lc_index, sqlite_orm::primary_key()),
          sqlite_orm::make_column("marker", &DBOPLCHead::marker),
          sqlite_orm::make_column("start_date", &DBOPLCHead::start_date)),
    sqlite_orm::make_table(std::string(LC_ENTRIES_TABLE),
          ...
          sqlite_orm::primary_key(&DBOPLCEntry::lc_index, &DBOPLCEntry::bucket_name))
  );
}
