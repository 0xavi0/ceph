...
class SQLiteLifecycle {
 ...
 public:
  ...
  std::optional<DBOPLCEntry> get_entry(const std::string & oid,
                                      const std::string & marker) const;
  std::optional<DBOPLCEntry> get_next_entry(const std::string & oid,
                                           const std::string & marker) const;
  void store_entry(const DBOPLCEntry & entry) const;
  void remove_entry(const std::string & oid, const std::string & marker) const;

  std::vector<DBOPLCEntry> list_entries(const std::string & oid,
