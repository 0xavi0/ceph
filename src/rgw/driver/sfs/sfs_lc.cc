...
SFSLifecycle::SFSLifecycle(SFStore* _st)
  : store(_st) {
}

int SFSLifecycle::get_entry(const std::string& oid, const std::string& marker,
...
int SFSLifecycle::get_next_entry(const std::string& oid,
...
int SFSLifecycle::list_entries(const std::string& oid,
                               const std::string& marker,
...
int SFSLifecycle::get_head(const std::string& oid,
                           std::unique_ptr<LCHead>* head)
{
  rgw::sal::sfs::sqlite::SQLiteLifecycle sqlite_lc(store->db_conn);
...
std::unique_ptr<LCSerializer> SFSLifecycle::get_serializer(const std::string& lock_name,
                                                           const std::string& oid,
                                                           const std::string& cookie)
{
  // creates or returns the mutex in the map for the given oid
  std::timed_mutex & mutex = mutex_map[oid];
  return std::make_unique<LCSFSSerializer>(mutex, store, oid, lock_name, cookie);
}
